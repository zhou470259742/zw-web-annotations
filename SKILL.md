...
5. 并行以**任务文件**为扇出单元：一个页面一个子 agent，同页内的多项任务归同一 agent（按任务 ID 再拆会并发改同一批源码）。状态分工：**子 agent 自行回写 `doing`/`review`**（服务端已把写操作串行化，并发安全），**`done` 与归档集中在主线程**——浏览器验收也只在主线程做（通常只有一个浏览器标签页，多个 agent 同时操作会互相抢占）。收尾顺序：子 agent 各自置 `review` → 主线程逐页验收通过的改 `done` → 归档；
