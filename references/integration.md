...
运行时安装到 `<项目>/.zcode/web-annotations/runtime/` 后，各框架的接入方式如下。
...
1. **接口**：提供 `/__zcode/annotations/*`，把任务写进 `.zcode/web-annotations/tasks/`；
...
export default {
  plugins: [zcodeAnnotations({ dir: '.zcode/web-annotations/tasks' })],
};
```
...
// src/main.js
import Vue from 'vue';
import { createAnnotations } from './.zcode/web-annotations/runtime/adapters/vue2.mjs';
...
注意：适配器只负责挂 UI，**不提供写盘接口**。用了适配器仍然要配中间件提供 `/__zcode/annotations/*`。
...
```js
const { createAnnotationsMiddleware } = require('./.zcode/web-annotations/runtime/adapters/http.mjs');
  ...
  devServer: {
    setupMiddlewares(middlewares, devServer) {
      devServer.app.use(createAnnotationsMiddleware({
        workspace: __dirname,
        dir: '.zcode/web-annotations/tasks',
      }));
      return middlewares;
    },
  },
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `route` | `/__zcode/annotations` | 接口前缀 |
| `clientPath` | `/__zcode/annotations/client.js` | 组件脚本路径 |
| `workspace` | `process.cwd()` | 工作区根目录，决定任务落盘位置 |
| `dir` | 空（回退到 `.zcode/web-annotations/tasks`） | 任务目录，相对 workspace 解析 |
...
```js
import { injectAnnotatorScript } from './.zcode/web-annotations/runtime/adapters/http.mjs';
...
接入构建配置时若有备份（`*.zcode-backup`），复制回原名即可：

```bash
cp vite.config.ts.zcode-backup vite.config.ts
```

完全移除：删除 `.zcode/web-annotations/` 目录，并撤销对构建配置的改动。
